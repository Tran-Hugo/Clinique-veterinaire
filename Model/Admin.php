...
class Admin extends Blog
{
  ...
  /* ========== UPDATE ========== */
    ...
    {
      $oStmt = $this->oDb->prepare('UPDATE animal SET nom = :nom, DateNaissance = :DateNaissance WHERE id = :postId LIMIT 1');
      $oStmt->bindValue(':postId', $aData['post_id'], \PDO::PARAM_INT);
      $oStmt->bindValue(':nom', $aData['nom'], \PDO::PARAM_STR);
      $oStmt->bindValue(':DateNaissance', $aData['DateNaissance'], \PDO::PARAM_LOB);
    ...
    {
      $oStmt = $this->oDb->prepare('DELETE FROM animal WHERE id = :postId LIMIT 1');
    ...
    {
      $oStmt = $this->oDb->prepare('INSERT INTO animal (nom, DateNaissance, id_user) VALUES(:nom, :DateNaissance,:id)');
      $oStmt->bindValue(':nom', $aData['nom'], \PDO::PARAM_STR);
      $oStmt->bindValue(':DateNaissance', $aData['DateNaissance'], \PDO::PARAM_LOB);
      $oStmt->bindValue(':id', $aData['id'], \PDO::PARAM_INT);
      return $oStmt->execute();
    } // ajoute les infos de l'animal + l'id de l'utilisateur qui l'a ajouté -> model/blog.php getAnimals()
