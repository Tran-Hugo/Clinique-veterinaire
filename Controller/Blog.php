...
class Blog
{
  ...
  public function index()
  {
       $this->oUtil->oPosts = $this->oModel->get(0, self::MAX_POSTS);
		$this->oUtil->oAnimals = $this->oModel->getAnimals($_SESSION['is_user_id']);//utilise la fonction de model/blog.php getAnimals() et stock les info dans un tableau oAnimals -> index.php
			...
			if (isset($_POST['submit']))
			{
				...
				else
				{
					if ($oIsAdmin->admin != null)
					{
						...
						// $_SESSION['is_admin_id'] = $oIsAdmin->id;
						$_SESSION['is_user_id'] = $oIsAdmin->id;
						header('Location: ' . ROOT_URL . 'blog_index.html');
						exit;
					}
					else
					{
						...
						$_SESSION['is_user_id'] = $oIsAdmin->id; // récupère l'id au moment de se connecter -> Controller / Admin.php add()
						header('Location: ' . ROOT_URL . 'blog_index.html');
						exit;
					}
				}
